    ...
    while run:
        ...
        if len(birds) > 0:
            for i, pipe in enumerate(pipes):
                if pipe.passed and birds[0].x > pipes[0].x + pipes[0].PIPE_TOP.get_width():
                    pipe_ind = i+1
                    break
        else:
            run = False
        ...
        add_pipe = False
        rem = []
        if lastPipe.x < 500:  # pipeTime > 1.3 and
            add_pipe = True
        ...
        for pipe in pipes:
            ...
            if pipe.x + pipe.PIPE_TOP.get_width() < 0:
                rem.append(pipe)
            pipe.move()
        pipeTime = time.time() - lastPipeCreationTime
        if add_pipe:
